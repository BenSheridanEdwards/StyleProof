...
async function captureColdBase() {
  ...
  if (isPublishablePartial(baseDirPath)) {
    log(
      `base capture exited ${status} with a publishable partial — keeping survivors (Soft-pass HOLD: stay red)`,
    );
    recordBaselineProvenance({ baseline: 'captured' });
    return false;
  }
  ...
  if (baseHit && headHit) {
    ...
    if (status !== 0) {
      ...
      if (isPublishablePartial(headDir)) {
        log(
          `head capture exited ${status} with a publishable partial — keeping survivors (Soft-pass HOLD: stay red)`,
        );
