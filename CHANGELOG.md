...
> **Prepare-only.** Package metadata and changelog for tip since `v7.0.1` /
> `c8d5e808`. Does not publish npm or move `@v7` — operators run `release.yml`
> after MERGE-READY.

### Added

- **CLI suite cannot hang forever on a leaked-handle child (#711).**
  `spawnSyncBounded` in the test harness applies a 60s wall-clock timeout to
  CLI `spawnSync` children, and `scripts/run-node-test.mjs` adds
  `--test-timeout=300000` on Node 20+ (omitted on Node 18, which rejects the
  flag). A hung child becomes a bounded `ETIMEDOUT` failure instead of stalling
  `npm test` / pre-push indefinitely.
...
- **Artifact-mode approval binds the uploaded bytes (#703).** The Action writes
...
- **ADR: keep commit statuses over the Checks API (#707).** Design note that
  StyleProof continues to publish commit statuses rather than Checks API
  check-runs for gate signalling.
...
- `styleproof-init` now activates its generated `.githooks/pre-push` shim only
  ...
  pre-push hook is absent. Hook ownership requires exact generated bytes;
  symlinked leaf destinations or parent directories, dangling links, directories,
  unreadable, malformed, or ambiguous hook states fail closed and are never
  followed or overwritten. Effective
  system/global/local/worktree/custom/Husky precedence is preserved without
  enabling `extensions.worktreeConfig`, while verified matching hooks remain
  non-destructive under init, `--hook`, `--check`, and `--upgrade`. Default Git
  hook parent components are classified without following links inside the Git
  common directory, so malformed `.git/hooks` parents cannot be mistaken for an
  absent hook and trigger activation. Hook status checks use the same boundary, so
  an executable behind a symlinked generated parent is never reported as an active
  repository-owned hook. Generated `.gitignore` updates use the same no-follow
  classification and exact UTF-8 byte validation, so symlinked, dangling,
  directory, unreadable, read-only, malformed, or permission-denied destinations
  are left byte-for-byte unmanaged. All generated writes catch residual filesystem
  denial rather than crashing. Normal init also reports unsafe spec, config,
  and workflow destinations explicitly as unmanaged rather than ordinary existing
  files (#409).
...
- **`styleproof-init` now activates the pre-push hook in the safe scope**. A
  single-worktree repository uses local config; repositories with linked
  worktrees use worktree-scoped config only when `extensions.worktreeConfig` is
  already enabled. Ambiguous or repository-owned hook state remains inactive
  with an explicit remediation. Automatic ownership requires exact generated
  bytes, while `styleproof-init --hook` is the explicit adoption path.
