...
- `report.md` and `report.json` are readable from the immutable publication — the publication commit in branch mode, or the run's `styleproof-report-pr-<number>` artifact zip in artifact mode — the JSON parses, and the publisher's receipt in Markdown exactly matches the comment's source head, run ID, and run attempt, and the immutable JSON records the canonical `STYLE_REVIEW_REQUIRED` machine verdict;
