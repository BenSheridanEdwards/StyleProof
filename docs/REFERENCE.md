...
## Map-restore hit observability (Soft-pass HOLD)
...
styleproof: map-restore side=base sha=<requested-sha> base_hit=ancestor ancestor_reuse_from=<ancestor-sha>
styleproof: map-restore side=base sha=<full-sha> base_hit=miss cold_reason=<enum>
```

`base_hit` is the restore-decision vocabulary (`exact` | `ancestor` | `miss`) for
either side (`side=base` or `side=head`). Soft-pass HOLD: these lines are
**observe only** — they never soft-green, skip-as-pass, or weaken gates.
...
**[Forks and Dependabot](#forks-and-dependabot)**. Fork verdicts are advisory
and need a maintainer's approval to turn green. The opt-in `--storage branch`
...
**[Open the full generated report](demo/report.md)** ([`report.html`](demo/report.html) — download and open for the rendered page).
...
Each discovered link becomes a surface keyed by its URL (`/?tab=overview` → `overview`; pass `key` for a different scheme). Replay, self-check and clock-freeze behave exactly as for explicit surfaces; one Playwright test runs the whole sweep (the link set isn't known until the page renders).

**Client-routed views (no `<a href>`):** the crawl also observes programmatic navigation — `history.pushState`, `replaceState`, and `popstate`, the APIs every client router funnels through — and adds each newly observed same-origin route to the surface set. A route the app lands on during hydration (a router's default redirect, a `settle` hook that clicks a client-routed tab) is discovered and captured without a router-specific adapter. In-page `<a href="#section">` anchors never call the history API, so anchor noise cannot become surfaces. Discovery is bounded (up to 64 observed routes over up to 3 passes) and each addition is logged (`navigation observer discovered N route(s)`); observed routes join the `expected`/`exclude` reconciliation below. Set `observeNavigation: false` to crawl rendered links only. One boundary stands: a nav that neither links nor navigates during the crawl still exposes nothing.
...
| Live / volatile regions (tickers, third-party embeds)       | auto-detected as still-moving and excluded from direct element comparison; head-only volatility blocks           |
...
Under branch storage, StyleProof publishes to `styleproof-maps` and
`styleproof-reports` branches — the default artifact storage creates neither.
These artifact branches contain JSON maps and Markdown reports, not deployable
code. Vercel auto-deploys every branch push by default, including these artifact
branches, which wastes build minutes and fails with confusing framework errors.
...
## Optional: selective remap (advisory + CI opt-in)
...
It is **opt-in and never the default gate** — without opt-in the gate still captures every surface and lets the map be the oracle. Soft-pass HOLD: selective remap never soft-greens; compare stays fail-closed on real diffs. The engine is built to be wrong only in the safe direction: when it cannot _prove_ a surface is unaffected, it returns the sentinel `'all'` (re-capture everything). A global stylesheet or token, a vanilla (unscoped) stylesheet, a `createGlobalStyle`, a design-system config, an unbounded `import(x)`, or a file it can't place — all resolve to `'all'`.

**CI / Action map path:** set `STYLEPROOF_SELECTIVE_REMAP=1` (or `affected.selectiveRemap: true` in config) so `styleproof-ci` plans a selective head capture when a usable base is present and the verdict is scoped. It logs selective ON/OFF, recapture/reuse counts, and the fail-closed reason when remapping all. Missing graph/surfaces/base or an unbounded verdict falls back to a full remap — never silent under-capture. Requires the same `affected.surfaces` / `affected.graph` inputs as `styleproof-affected`.
...
| `report-retention-days`  | `30`                         | Days the report artifact is kept when `report-storage` is `artifact`. Must be a positive integer in either storage mode; `0` is rejected. GitHub caps retention at 90 days unless repository settings allow more. |
...
- The commit status is set when `require-approval` is `true` or `mode` is `review-gate`, `migration`, or `advisory`. In advisory mode (or when the config sets `blocking: 'advisory'`) the status is `success`, except for a fork PR whose maps came from the untrusted capture job: that status is `pending` until a maintainer approves.
...
| `affected`                | —                        | `{ "surfaces": { key: entryModulePath }, "graph": "dc.json", "base": "origin/main", "selectiveRemap": false }` — pins `styleproof-affected`'s inputs so a configured repo runs it bare; `selectiveRemap: true` (or `STYLEPROOF_SELECTIVE_REMAP=1`) opts `styleproof-ci` into fail-closed selective head remap when base maps are present.                                                                                                                                    |
...
| `STYLEPROOF_NAVIGATE_TIMEOUT_MS`       | Navigate-phase budget in ms (default `60000`, capped by the overall surface ceiling); a stuck navigate fails that surface early with phase `navigate` named.                                                                                                          |
...
| `STYLEPROOF_SELECTIVE_REMAP`           | `1`/`true` to opt `styleproof-ci` into selective head remap (fail-closed to full remap when base/graph/surfaces/verdict are missing or unbounded). Default off. Config: `affected.selectiveRemap`. Soft-pass HOLD.                                                  |
...
| `--workflow single\|split` | `single` | One in-job workflow, or the two-stage layout for fork/Dependabot PRs (fork verdicts need maintainer approval). |
