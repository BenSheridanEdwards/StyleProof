...
Configuration is static JSON. `styleproof.config.ts`, `.mjs`, and `.js` are unsupported and rejected without execution, even when a JSON config also exists. Move their configuration values into `styleproof.config.json` and remove the unsupported module files. Resolution stays in the selected working directory; no parent-directory search or config merging occurs. Fresh `styleproof-init` does not generate a project config file; create the optional JSON file when you need to override defaults:
