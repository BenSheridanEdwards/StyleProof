...
`styleproof.config.ts` is the supported format with full IDE autocomplete. Legacy `styleproof.config.json` remains supported with a deprecation warning directing migration to the typed config:
...
| `suppressPlatformWarning` | `true`                   | Suppress the platform mismatch warning when uploading maps from non-Linux local runs. On by default; set `false` to re-enable the warning. `STYLEPROOF_SUPPRESS_PLATFORM_WARNING=0` overrides the default at runtime (explicit config takes precedence over env).                                                                                                                                                |
