...
[![A real StyleProof report rendered on GitHub: before and after crops plus magenta change annotations](docs/readme/report-diff.png)](docs/demo/report.md)

_A real report generated by StyleProof and rendered on GitHub. Reviewers see the
unannotated before/after crop and the highlighted evidence, not a pass/fail score
alone._
...
These are current outputs generated by StyleProof and rendered on GitHub. The
clean certification comes from two real Chromium captures of this repository's
...
it or acknowledge it in policy; checking an approval box cannot clear the
failure.

### A visual change becomes reviewable evidence

[![The top of a real StyleProof report showing a new surface and the total computed-style differences](docs/readme/report-overview.png)](docs/demo/report.md)

The report leads with new surfaces and a concise change count, then groups each
distinct change with before/after crops, annotations, a plain-English summary,
and the exact computed properties.
...
| `STYLEPROOF_ANCESTOR_BASELINE_ROOTS` | Comma-separated repo-relative app source directories (e.g. `src,styles`) whose changes are capture-relevant to the ancestor-reuse gate. With no roots declared, every changed path counts as relevant. |
