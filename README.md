...
`styleproof.config.ts` is the supported format with full IDE autocomplete. Plain Node cannot evaluate a `.ts` file unless type stripping is on (Node 22.18+ default, or `--experimental-strip-types`) **and** the `styleproof` package is resolvable from that file. A CI worktree or first-adoption checkout that has the init scaffold but no install falls back to sibling JSON or an empty config with a warning — it does not crash, and it does not invent `e2e/styleproof.spec.ts` when a parent config already declared a spec. Runtime formats that always load are `.json` / `.mjs` / `.js`. Legacy `styleproof.config.json` remains supported with a deprecation warning directing migration to the typed config:
