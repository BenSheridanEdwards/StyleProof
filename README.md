...
> **Capture against a production build, not a dev server.** `next dev` / `vite` JIT-compile each route on first request — slow and timing-variable under parallel CI load, the single most common source of capture flakes. The network-aware settle above absorbs most of it, but a production build is faster and removes the variance entirely. Point your Playwright `webServer` at `next build && next start` (or `vite build && vite preview`), not the dev server. StyleProof prints a one-time warning if it detects it's capturing against a dev server.
