...
Setup checks that a production server command can be inferred before installing or
scaffolding. Installation, server commands and CI share package-manager detection;
a supported `packageManager` declaration takes precedence over stale lockfiles.
It does not execute the server or prove application readiness. Unknown
projects without a `start` or `preview` script must provide `--server-command` or
use `--external-server` for an already-managed URL. See the [setup server contract](docs/setup.md).

Requires **Node ≥ 18** (ESM). Forced states are Chromium-only.

Forced interaction capture includes sibling and ancestor CSS effects, not just
the target subtree. Incomplete, truncated, detached, or explicitly disabled state
evidence cannot certify fully or be cleared by visual approval. The scan has a
bounded work budget; see [forced-state capture](docs/forced-state-capture.md) for
limits. Regenerate existing baselines after upgrading so both sides use the same
expanded state-capture semantics.
...
`styleproof.config.ts` is the supported format with full IDE autocomplete. Plain Node cannot evaluate a `.ts` file unless type stripping is on (Node 22.18+ default, or `--experimental-strip-types`) **and** the `styleproof` package is resolvable from that file. If that file is found but cannot be evaluated, StyleProof fails closed and names the path and the reason — it does not invent `{}` or `e2e/styleproof.spec.ts` while the file exists. A sibling `styleproof.config.json` in the same directory is the only fallback, and only when that JSON loads as the real config. Runtime formats that always load are `.json` / `.mjs` / `.js`. Legacy `styleproof.config.json` remains supported with a deprecation warning directing migration to the typed config:
