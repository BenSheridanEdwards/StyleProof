...
function generateStyleMapReportInternal(opts: ReportOptions, includeStructure: boolean): ReportResult {
  ...
  if (missing.length > 0) {
    appendWithinBudget(['', '## One-sided pages, states, or surfaces — review first']);
  }
  ...
  if (orderedGroups.length > 0) {
    appendWithinBudget(['', '## Element-level changes']);
  }
  ...
  if (contentSection.md.length > 0) {
    emitDetail(
      contentSection.md,
      `- ${contentSection.count} advisory content/structure change(s); full evidence is in report.json and crops/`,
    );
  }
