...
## The data-residue guard — `dataResidue`

A third guard, for the surface that renders the **wrong** state silently: if a
data request under `replayUrl` (default `**/api/**`) *fails* during capture, the
surface paints its fallback branch and the response-driven state is never
certified. Warned always and **gates by default** (v4) — an unacknowledged
failure blocks (ack ledger `styleproof.data-residue.json`); `dataResidue: 'warn'`
opts down to record-only. The failing-request half of the un-exercised-state
gap — `styleproof-coverage` covers both halves.
