...
test('a full-page non-body shell is not selected as content context', () => {
  const map = (text) =>
    makeMap({
      elements: {
        body: { tag: 'body' },
        'body > main:nth-child(1)': { tag: 'main', cls: 'page', rect: [0, 0, 450, 220] },
  ...
  assert.equal(composite.width, 468, 'the 94%-wide and 92%-tall page shell does not replace the leaf crop');
  rmTmp(dirs.root);
});
