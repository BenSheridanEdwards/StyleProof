...
// ── Nearest-ancestor baseline reuse (#367, #595 — enabled by default, opt-out):
...
// Prior to #595 this was opt-in via STYLEPROOF_ANCESTOR_BASELINE=1; now default-on.
// #595: ancestor baseline is ENABLED BY DEFAULT — no env var needed, uses ['src'] roots
test(
  'styleproof-ci: ancestor baseline reuse is enabled by default without env vars (#595)',
  { timeout: 60_000 },
  () => {
    const root = mkTmp('styleproof-ci-ancestor-default-');
    const remote = path.join(root, 'remote.git');
    const repo = path.join(root, 'consumer');
    const mapRoot = path.join(root, 'maps');
    const githubOutput = path.join(root, 'github-output');
    ...
    try {
      ...
      git(repo, ['config', 'user.name', 'StyleProof Test']);
      git(repo, ['remote', 'add', 'origin', remote]);
      // Ancestor A: the commit whose bundle is stored.
      ...
      // Base B: a docs-only merge on top of A — nothing in the default ['src'] roots changed.
      ...
      // NO STYLEPROOF_ANCESTOR_BASELINE env var — default is enabled with ['src'] roots
      const result = runCi(
        ['--base', base, '--head', head, '--spec', 'styleproof.spec.ts', '--base-dir', mapRoot, '--force'],
        {
          CI: '1',
      ...
      assert.equal(result.status, 0, result.stderr + result.stdout);
      assert.match(result.stderr, /reused the baseline of nearest ancestor/, 'ancestor reuse fires by default (#595)');
      const outputs = fs.readFileSync(githubOutput, 'utf8');
      assert.match(outputs, new RegExp(`base-restored-from-ancestor=${ancestor}`), 'default-on reuse is auditable');
    } finally {
      rmTmp(root);
    }
  },
